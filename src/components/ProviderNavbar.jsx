...
function ProviderNavbar({ currentView, setCurrentView, setIsLoggedIn }) {
  const { toast } = useToast()
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  const navItems = [
    { icon: Bell, view: "notificationsprovider", label: "الإشعارات" },
    { icon: Users, view: "usersRequest", label: "المستخدمين" },
    { icon: Wallet, view: "walletprovider", label: "طلبات السحب" },
    { icon: DollarSign, view: "transferprovider", label: "المعاملات" },
    ...
    <nav className="fixed top-0 left-0 right-0 bg-primary text-primary-foreground shadow-sm z-50">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <button 
            onClick={() => handleViewChange("walletprovider")}
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem
                  onClick={() => handleViewChange("providersettings")}
